...
def test_pipeline_end_to_end_dry_run(tmp_path):
    ...
    broker = AlpacaPaperBroker(ex, dry_run=True, store_path=tmp_path / "intents.json")
    ...
    def ai_fn(ctx, p, q):
        ...
        parsed = parse_ai_json(
            '{"action":"HOLD","confidence":0.4,"trend":"SIDEWAYS",'
            '"entry_quality":"WEAK","reasoning_summary":"mixed","risk_notes":[]}')
        return AIAnalysis(ctx.symbol, parsed.action, parsed.confidence,
                          prompt_version=PROMPT_VERSION)

    deps = CycleDeps(
        settings=settings, provider=provider, indicators=indicators,
        build_snapshot_fn=build_snapshot, detect_regime_fn=detect_regime,
        ai_decide_fn=ai_fn, confluence_fn=evaluate,
        risk_decide_fn=lambda i: risk_decide(RiskConfig(), i), broker=broker,
        snapshot=PortfolioSnapshot(equity=100000.0, broker_ok=True),
        min_confidence=0.5)
    ...
    assert res.signal.hold_reason == "AI_HOLD"
...
def test_stale_data_holds_without_trading():
    ...
    deps = CycleDeps(
        settings=MagicMock(trend_interval="5Min", trend_lookback=60,
                           entry_interval="1Min", entry_lookback=40),
        provider=provider, indicators=MagicMock(), build_snapshot_fn=build_snapshot,
        detect_regime_fn=detect_regime, ai_decide_fn=MagicMock(),
        confluence_fn=evaluate, risk_decide_fn=MagicMock(),
        broker=MagicMock(), snapshot=PortfolioSnapshot(equity=100000.0),
        min_confidence=0.5)
    res = run_cycle(deps, "BTC/USD")
    assert res.final_state == "HOLD" and any("stale" in e or "market_quality" in e for e in res.errors)
    assert res.signal.hold_reason == "STALE_DATA"
